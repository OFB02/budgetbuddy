  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const csvContent = [
        ...
        `Vacation Type: ${vacationType || 'General'}`,
        ...
        '',
        'BUDGET CATEGORIES',
        'Category,Amount,Per Person,Per Day',
        `Flights,${flights.toFixed(2)},${(flights / travelers).toFixed(2)},-`,
        `Accommodation,${accommodation.toFixed(2)},${(accommodation / travelers).toFixed(2)},${(accommodation / duration).toFixed(2)}`,
        `Food & Dining,${foodTotal.toFixed(2)},${(foodTotal / travelers).toFixed(2)},${(foodTotal / duration).toFixed(2)}`,
        `Activities & Tours,${activities.toFixed(2)},${(activities / travelers).toFixed(2)},${(activities / duration).toFixed(2)}`,
        `Local Transportation,${transportation.toFixed(2)},${(transportation / travelers).toFixed(2)},${(transportation / duration).toFixed(2)}`,
        `Shopping & Souvenirs,${shopping.toFixed(2)},${(shopping / travelers).toFixed(2)},${(shopping / duration).toFixed(2)}`,
        '',
        'BUDGET SUMMARY',
        `Subtotal,${totalBudget.toFixed(2)}`,
        `Emergency Fund (${emergencyRate}%),${emergencyFund.toFixed(2)}`,
        `Total Budget,${grandTotal.toFixed(2)}`,
        '',
        'SAVINGS PLAN',
        `Current Savings,${currentSaved.toFixed(2)}`,
        `Amount Needed,${amountNeeded.toFixed(2)}`,
        `Months to Save,${monthsToSave}`,
        `Monthly Savings Required,${monthlySavings.toFixed(2)}`,
        '',
        'PER PERSON BREAKDOWN',
        `Total Per Person,${budgetPerPerson.toFixed(2)}`,
        `Daily Budget Per Person,${(budgetPerDay / travelers).toFixed(2)}`,
        '',
        'Generated by BudgetBuddy',
      ].join('\n');

      const fileUri = FileSystem.documentDirectory + `vacation_budget_${destination.replace(/\s+/g, '_')}_${Date.now()}.csv`;
      ...
      Alert.alert('Export Failed', 'Unable to export budget. Please try again.');
    } finally {
